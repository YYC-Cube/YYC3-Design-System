...
export class AITokenGenerator {
  ...
  private generateComplementary(baseColor: string): string[] {
    const hsl = culori.converter('hsl')(baseColor) as { h: number; s: number; l: number; mode?: 'hsl' } | null;
    ...
    if (colors.length > 0) {
      const colorHexes = colors.map(c => {
        ...
        if (typeof c === 'object' && c !== null && 'value' in c) {
          ...
          if (typeof value === 'object' && value !== null && 'hex' in value) return (value as { hex: string }).hex;
        }
        return c as string;
      });
