  ...
  .action((options) => {
    ...
    try {
      ...
      const auditReport = {
        ...
        overall: {
          score: Math.round((consistencyReport.overallScore + usageReport.summary.coverage) / 2),
          status:
            consistencyReport.overallScore >= 80 && usageReport.summary.coverage >= 80
              ? 'healthy'
              : 'needs-attention',
        },
      };
      ...
      console.log(
        `  状态: ${auditReport.overall.status === 'healthy' ? '✅ 健康' : '⚠️ 需要关注'}`
      );
