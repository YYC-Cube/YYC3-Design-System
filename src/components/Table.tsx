      ...
      (record: T, checked: boolean) => {
        ...
        setSelectedRowKeys((prev) => {
          ...
          rowSelection?.onChange?.(
            newKeys,
            newKeys.map((k) => dataSource.find((r, i) => getRowKey(r, i) === k)!)
          );
          return newKeys;
        });
      },
      ...
      <div style={{ position: 'relative' }} className={className} data-testid={dataTestId}>
        ...
        {pagination && (
          ...
          >
            ...
            <span>第 {pagination.current || 1} 页</span>
            <button
              onClick={() =>
                pagination.onChange?.((pagination.current || 1) + 1, pagination.pageSize || 10)
              }
              disabled={
                !pagination.total ||
                (pagination.current || 1) * (pagination.pageSize || 10) >= pagination.total
              }
              style={{
                padding: '4px 8px',
                border: `1px solid ${(tokens['color.border'] as string) || '#d0d0d0'}`,
            ...
            <button
              onClick={() => {
                const totalPages = Math.ceil(
                  (pagination.total || dataSource.length) / (pagination.pageSize || 10)
                );
            ...
            {pagination.showQuickJumper && (
              <span>
                ...
                <input
                  ...
                  min={1}
                  max={
                    pagination.total ? Math.ceil(pagination.total / (pagination.pageSize || 10)) : 1
                  }
                  style={{
                    width: '50px',
                    padding: '4px 8px',
