...
export const Menu = memo<MenuProps>(({
  ...
  const containerStyles: React.CSSProperties = {
    ...
    backgroundColor: isDark ? tokens['color.background.dark'] as string || '#1a1a1a' : tokens['color.background'] as string || '#fff',
    border: mode === 'vertical' ? `1px solid ${tokens['color.border'] as string || '#e0e0e0'}` : undefined,
    borderRadius: '4px',
    overflow: 'hidden',
  };
  ...
  const itemStyles: React.CSSProperties = {
    ...
    transition: 'all 0.2s',
    fontSize: '14px',
    color: isDark ? tokens['color.text.dark'] as string || '#fff' : tokens['color.text.primary'] as string || '#333',
  ...
  const itemHoverStyles: React.CSSProperties = {
    ...itemStyles,
    backgroundColor: isDark ? 'rgba(255, 255, 255, 0.1)' : tokens['color.hover'] as string || '#f5f5f5',
  };

  const selectedItemStyles: React.CSSProperties = {
    ...itemStyles,
    backgroundColor: isDark ? 'rgba(255, 255, 255, 0.15)' : tokens['color.primary.light'] as string || '#f0f0f0',
    borderLeftColor: mode === 'vertical' ? tokens['color.primary'] as string || '#d45a5f' : undefined,
    borderBottomColor: mode === 'horizontal' ? tokens['color.primary'] as string || '#d45a5f' : undefined,
  ...
  const iconStyles: React.CSSProperties = {
    fontSize: '16px',
    color: isDark ? tokens['color.text.dark'] as string || '#fff' : tokens['color.text.secondary'] as string || '#666',
    minWidth: '16px',
  };
  ...
  const shortcutStyles: React.CSSProperties = {
    marginLeft: 'auto',
    fontSize: '12px',
    color: isDark ? tokens['color.text.disabled.dark'] as string || '#666' : tokens['color.text.disabled'] as string || '#999',
  };
  ...
  const renderMenuItem = (item: MenuItem, level = 0): ReactNode => {
    ...
    const itemContent = (
      <>
        ...
        {!isCollapsed && hasChildren && <span style={{ marginLeft: 'auto', fontSize: '12px' }}>{isExpanded ? '▼' : '▶'}</span>}
      </>
    );
    ...
    return (
      <div key={item.key}>
        <div
          style={{
            ...(item.disabled ? disabledItemStyles : isSelected ? selectedItemStyles : baseItemStyles),
