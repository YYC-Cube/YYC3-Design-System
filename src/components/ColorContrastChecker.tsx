...
export const ColorContrastChecker: React.FC<ColorContrastCheckerProps> = ({ className = '' }) => {
  ...
  const wcagRating = useMemo(() => {
    ...
    if (contrastRatio >= 3)
      return { level: 'AA Large', small: false, large: true, graphics: false };
  ...
  return (
    <div style={containerStyle} className={className}>
      ...
      >
        <div style={textStyle}>This is normal text size (16px)</div>
        <div style={largeTextStyle}>This is large text size (24px)</div>
        <div style={textStyle}>
      ...
      <div style={resultsStyle}>
        <div style={ratioStyle}>Contrast Ratio: {contrastRatio.toFixed(2)}:1</div>
        <div style={ratingStyle}>WCAG Rating: {wcagRating.level}</div>
