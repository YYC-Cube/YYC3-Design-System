...
import type { ResponsiveValue } from '../types/advanced-types';
...
export type TokenAccessor<T extends object = DesignTokens> = {
  ...
  get3<K1 extends keyof T, K2 extends keyof T[K1], K3 extends keyof T[K1][K2]>(
    key1: K1,
    key2: K2,
    key3: K3
  ): T[K1][K2][K3];
...
export type TokenResponsive = {
  getResponsive(path: TokenPath, breakpoints: ResponsiveValue<TokenValue>): string;
...
export type TokenTheme<T extends object = DesignTokens> = {
  getThemeValue(path: TokenPath, theme: T): TokenValue;
  createTheme(base: T, overrides: Partial<T>): T;
...
export type TokenCSS = {
  ...
  createCSSProperties(paths: TokenPath[], prefix?: string): React.CSSProperties;
};

export const getTokenPath = (path: TokenPath): string[] => {
...
export const hasToken = <T extends object = DesignTokens>(obj: T, path: TokenPath): boolean => {
  return getNestedValue(obj, path) !== undefined;
};

export const isTokenColor = <T extends object = DesignTokens>(obj: T, path: TokenPath): boolean => {
...
export const isTokenSize = <T extends object = DesignTokens>(obj: T, path: TokenPath): boolean => {
  const value = getNestedValue(obj, path);
  if (typeof value !== 'string') return false;
...
export const transformColor = (value: string, format: 'hex' | 'rgb' | 'hsl' | 'oklch'): string => {
  if (value.startsWith('#')) {
...
export const transformSize = (value: string | number, unit: 'px' | 'rem' | 'em'): string => {
...
export const transformSpacing = (value: string, unit: 'px' | 'rem' | 'em'): string => {
  if (value.match(/^\d+$/) || value.match(/^\d+\.\d+$/)) {
    return `${value}${unit}`;
  }
...
export const createTokenCSS = <T extends object = DesignTokens>(
  ...
  return {
    ...
    createCSSProperties(paths: TokenPath[], customPrefix?: string): React.CSSProperties {
      const actualPrefix = customPrefix || prefix;
        ...
        breakpoints.forEach((bp) => {
          if (typeof bp === 'object' && bp !== null) {
            ...
            bpKeys.forEach((key) => {
              ...
              if (k in bp) {
                (style as Record<string, string>)[`${property}-${k}`] = getResponsiveValue(
                  path,
                  bp[k] as ResponsiveValue<TokenValue>
                );
              }
            });
          }
...
export const createTokenTheme = <T extends object = DesignTokens>(tokens: T): TokenTheme<T> => {
  const getThemeValue = (path: TokenPath, theme: T): TokenValue => {
    return getNestedValue(theme, path) as TokenValue;
  };

  const createTheme = (base: T, overrides: Partial<T>): T => {
...
export const useToken = <T extends object = DesignTokens>(tokens: T) => {
